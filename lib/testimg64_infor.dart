...
class Test_iminfr extends StatefulWidget {
  ...
  const Test_iminfr({
    ...
    required this.userCardImagePath,
    required this.imageFile,
  });

  @override
  State<Test_iminfr> createState() => _Test_iminfrState();
}

class _Test_iminfrState extends State<Test_iminfr> {
...
//===============================================
  ...
  Future<void> insert_Register(
    ...
    String password,
    File? imgIdCard,
    File? imgDriverLicense,
    File? imgCarPicture,
    File? imgCarRegistration,
    File? imgAct,
    ...
    try {
      ...
      String? base64ImgProfile = imgProfile != null
          ? base64Encode(await imgProfile.readAsBytes())
          : null;
      String? base64ImgIdCard = imgIdCard != null
          ? base64Encode(await imgIdCard.readAsBytes())
          : null;
      String? base64ImgDriverLicense = imgDriverLicense != null
          ? base64Encode(await imgDriverLicense.readAsBytes())
          : null;
      String? base64ImgCarPicture = imgCarPicture != null
          ? base64Encode(await imgCarPicture.readAsBytes())
          : null;
      String? base64ImgCarRegistration = imgCarRegistration != null
          ? base64Encode(await imgCarRegistration.readAsBytes())
          : null;
      String? base64ImgAct =
          imgAct != null ? base64Encode(await imgAct.readAsBytes()) : null;

      var response = await http.post(
        Uri.parse('http://${Global.ip_8080}/gotwo/rider_Register.php'),
        body: {
          "img_profile": base64ImgProfile,
          "name": name,
          "email": email,
          "tel": tel,
          "gender": gender,
          "password": password,
          "img_id_card": base64ImgIdCard,
          "img_driver_license": base64ImgDriverLicense,
          "img_car_picture": base64ImgCarPicture,
          "img_car_registration": base64ImgCarRegistration,
          "img_act": base64ImgAct,
      ...
      if (response.statusCode == 200) {
        debugPrint("Insert success: ${response.body}");
      } else {
    ...
    return ElevatedButton(
      onPressed: () {
        ...
        Future<void> insert_Register(
          ...
          try {
            // แปลงไฟล์ภาพเป็น Base64 หากไฟล์ไม่ใช่ null
            String? base64ImgProfile = imgProfile != null
                ? base64Encode(await imgProfile.readAsBytes())
                : null;
            String? base64ImgIdCard = imgIdCard != null
            ...
            String? base64ImgCarRegistration = imgCarRegistration != null
                ? base64Encode(await imgCarRegistration.readAsBytes())
                : null;
            String? base64ImgAct = imgAct != null
                ? base64Encode(await imgAct.readAsBytes())
                : null;
